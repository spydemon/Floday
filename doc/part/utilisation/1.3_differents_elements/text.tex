...
L'\gls{instantiation} elle aussi est hiérarchique~: dans un premier temps, l'hôte sera déployé, puis viendra le tour de \emph{web\_application} et enfin celui de \emph{my\_blog} et de \emph{mum\_blog}.
...
\begin{description}
	\item[applications] défini toutes les \glspl{sous-application} qui seront gérées par l'application en cours de \gls{definition}.
...
Chaque script \path{setups}, \path{end-setup} ou tout autres types de scripts peuvent facilement être testés sans avoir à redéployer l'ensemble des applications.
