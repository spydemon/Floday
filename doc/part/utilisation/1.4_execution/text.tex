		...
		Pour chaque applications présentes dans l'hôte en cours de déploiement au niveau du runfile, on effectue les actions suivantes~:
			\begin{description}
				...
				\item[Pré-installation]
					On commencera par exécuter les scripts présent au niveau du hook \path{lxc-deploy-before}.
				...
				\item[Déploiement des contraints]
					Les applications \gls{contraint}es sont ensuite intégrallement déployées (elles passent donc elles aussi par l'intégralités des étapes de la partie \emph{Déploiement des applications} de cette liste.
				\item[Post-installation]
					Une fois tous les contraints correctement déployés, nous exécutons les scripts présents dans le nœud \emph{end\_setup}.
			\end{description}
\end{description}
